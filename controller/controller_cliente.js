    ...
    try {
        if (String(contentType).toLowerCase() == 'application/json') {
            ...
            if (
                ...
                if (novoCliente) {
                    novoClienteJSON.id = Number(idNovoCliente[0].id)
...
const setDeletarClientes = async function (id) {
    ...
    let validarID = await clienteDAO.selectByIdCliente(idClientes)
    if (validarID.length > 0) {

        if (idClientes == '' || idClientes == null || isNaN(idClientes)) {
            return message.ERROR_INVALID_ID //400
        }
        else {
            let deleteCliente = await clienteDAO.deleteCliente(idClientes)
